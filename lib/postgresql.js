...
var postgisTypes = require('pg-postgis-types').default;
...
exports.initialize = function initializeDataSource(dataSource, callback) {
  ...
  dbSettings.srid = dbSettings.srid || 4326;
...
PostgreSQL.prototype.connect = function(callback) {
  var self = this;
  // register postGIS types before connecting
  postgisTypes(self.pg, self.clientConfig, function (err, oids) {
    console.log(err, oids);
...
PostgreSQL.prototype.toColumnValue = function(prop, val) {
  ...
  if (prop.type.name === 'GeoPoint' || prop.type.name === 'Point') {
    return new ParameterizedSQL({
      ...
      params: [val.lng, val.lat, this.settings.srid]
    });
  }
